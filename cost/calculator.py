...
def prompt_rates(cached=None):
    """
    Prompts user for unit cost rates.
    If cached is provided, shows current values and allows accepting with enter.
    """
    ...
    fields = [
        ("storage_gb_month", "Storage cost per GB/month (USD)"),
        ("network_gb",       "Network transfer cost per GB (USD)"),
        ("cpu_hour",         "CPU cost per hour (USD)"),
        ("ram_gb_hour",      "RAM cost per GB/hour (USD)"),
    ]
...
def calculate_estimate(manifest_raw, selected_columns, rates,
                       expected_throughput_rps=50_000):
    """
    Returns estimate dict with per-category costs and a total.
    """
    ...
    col_fraction = max(col_fraction, 0.001)  # prevent zero-cost masking
    ...
    costs = {
        ...
        "storage_unoptimised_monthly": round(pg_unopt_gb * rates["storage_gb_month"], 4),
    ...
    if total_rows == 0:
        console.print("[yellow]Warning: total_rows not available — compute estimate is $0.00 (unreliable)[/yellow]")

    return costs


def display_estimate(costs):
    t = RichTable(title="Pre-transfer cost estimate (USD)", show_header=True)
    t.add_column("Item")
    t.add_column("Est. USD", justify="right")
    labels = {
        "storage_optimised_monthly": "PG storage/month (optimised)",
        "storage_unoptimised_monthly": "PG storage/month (unoptimised)",
        "network_transfer": "Network transfer",
        "compute_cpu": "Compute (CPU)",
        "compute_ram": "Compute (RAM)",
        "total_estimate": "[bold]Total[/bold]",
    }
