...
def decode_bit_array_columns(df, bit_cols, metadata):
    """
    For each column in bit_cols:
      - Unpack bytes into individual boolean columns as defined by metadata
      - Drop the original byte column
      - Insert new columns in its place
    ...
    for col in bit_cols:
        ...
        for out in output_defs:
            ...
            if bit_idx < unpacked.shape[1]:
                # Preserve null semantics: null input → pd.NA, not False
                ...
                logger.warning("bit_index %d out of range for column '%s'", bit_idx, col)
...
def _null_mask(series):
    """Returns boolean array: True where the original value was null."""
...
def _unpack_series(series, bit_width):
    """
    series: each value is bytes | None
    Returns ndarray of shape (len(series), bit_width), dtype uint8 (0 or 1).
    Null values become rows of zeros.
    """
