...
def create_table_from_df(engine, table_name: str, df: pd.DataFrame):
    ...
    logger.info("Creating table '%s' with %d columns", table_name, len(df.columns))
...
def _prepare_for_copy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        if dtype_str in (
            ...
            s_clean = (
                s_clean.str.replace(r"\\", r"\\\\", regex=True)
                .str.replace(r"\t", " ", regex=True)
                .str.replace(r"\n", " ", regex=True)
                .str.replace(r"\r", " ", regex=True)
            )
            df[col] = s_clean.where(mask, None)
...
def save_failed_chunk(df: pd.DataFrame, chunk_idx: int, exception: Exception):
    ...
    try:
        ...
        try:
            ...
            safe_df.to_csv(
                FAILED_DIR / f"failed_chunk_{chunk_idx}_numeric.csv", index=False
            )

        except Exception:
            logger.exception(
                "Could not save even numeric-only CSV for chunk %d", chunk_idx
            )
