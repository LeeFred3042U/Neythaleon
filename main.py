...
from ingest.config import DATABASE_URL, MANIFEST_FILE, MEDIA_DIR
...
def main():
    ...
    # ─── Phase 4: User confirmation + checkpoint ───
    confirm = Prompt.ask(
        "\n[bold]Proceed with ingestion?[/bold] [Y/n]",
        default="Y"
    )
    if confirm.upper() != "Y":
...
def phase_ingestion(selected_columns, bit_metadata, cp):
    ...
    console.print(f"[green]✓ Ingestion complete — {len(metrics)} chunks processed[/green]")
