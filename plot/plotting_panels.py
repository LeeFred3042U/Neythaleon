...
def _plot_time_series_with_gaps(
    ax,
    ts: pd.Series,
    y: pd.Series,
    *,
    ...
    for _, seg in df.groupby("_segment"):
        ...
        if len(seg) >= 3 and smooth_window > 1:
            ...
            ax.plot(
                seg["ts"], smoothed, linewidth=1.4, zorder=zorder_smooth, color=color
            )

        else:
            ax.plot(
                seg["ts"], seg["y"], linewidth=0.9, zorder=zorder_smooth, color=color
            )
...
def _annotate_peaks(ax, ts, y, top_n=2, label_fmt="{:.0f}"):
    ...
    try:
        ...
        for t in top_idx:
            ax.annotate(
                label_fmt.format(float(s.loc[t])),
                xy=(t, s.loc[t]),
                xytext=(5, 5),
...
def plot_cpu_vs_throughput(ax, df):
    ...
    try:
        denom = (
            float(np.nanpercentile(rps.dropna(), 75)) if not rps.dropna().empty else 1.0
        )
...
def plot_batch_time(ax, df):
    ...
    if len(x) > 1:
        ...
        median_diff_days = (
            float(np.nanmedian(diffs_days)) if len(diffs_days) > 0 else 0.0
        )
    ...
    try:
        ...
        upper = (
            max(5.0, float(np.nanpercentile(valid_y, 98) * 1.2))
            if valid_y.size
            else 5.0
        )
