...
NUMERIC_COLS = {
    "throughput_rps", "duration_sec", "cpu_percent",
    "memory_mb", "rows", "rows_per_second", "rows_processed"
}

def _maybe_coerce_numeric(s: pd.Series, thresh: float = 0.75) -> pd.Series:
    if s.dtype != object:
        return pd.to_numeric(s, errors="coerce") if pd.api.types.is_numeric_dtype(s) else s
...
def load_metrics(file_path: str | Path) -> pd.DataFrame:
    ...
    # Parse timestamp to datetimes (safe, coerce invalid to NaT)
    if "timestamp" in df.columns:
        df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")

    # Coerce well-known numeric metric columns
    for k in NUMERIC_COLS:
        if k in df.columns:
            df[k] = pd.to_numeric(df[k], errors="coerce")

    # Fuzzy-coerce any remaining object columns that are largely numeric
