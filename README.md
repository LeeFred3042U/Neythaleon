...
```bash
.
├── main.py                 # Entry point (runs ingest, then plot)
|
├── ingest/                 # Ingestion package
│   ├── __init__.py         # Exports configs + ingest_data
│   ├── cli.py              # CLI for ingestion
│   ├── config.py           # Configs, placeholders (DB, dirs, columns)
│   ├── ingest_runner.py    # Core ingestion logic
│   └── __pycache__/        # Auto-generated
├── plot/                   # Plotting package
│   ├── __init__.py
│   ├── cli.py              # CLI for dashboards/metrics plotting
│   ├── plot_runner.py      # Plotting logic
│   └── __pycache__/        # Auto-generated
├── media/                  # Autogenerated outputs
│   ├── ingestion_metrics.csv   # System + ingestion metrics
│   ├── performance_dashboard.png  # Example plot output
│   └── logs/
│       └── ingestion.log       # Step-by-step logs
├── parquet/                # Directory for source .parquet files
├── failed_chunks/          # Failed chunks from ingestion
├── .env.example                    # Database credentials (e.g., DATABASE_URL)
├── requirements.txt        
